...
public class Exercises {
	...
	public List<String> array2List(String[] stringArray) {
		...
		ArrayList<String> myList = new ArrayList<String>(stringArray.length);
	...
	public List<Integer> interleaveLists(List<Integer> listOne, List<Integer> listTwo) {
		...
		ArrayList<Integer> combindedList = new ArrayList<Integer>();

		for (Integer item : listOne) {
			combindedList.add(item);
		}
// make list 2 an array 
		// push values into combindedList at indexes 1, 3, 5, 7...)
//		int[] listTwoArray = new listTwo.toArray(); 
//
//		for (int i = 0; i < listTwoArray.length(); i++) {
//			listTwoArray[i].add(combindedList) ... not sure what to do here or if this will even work 
//		}		.get() ... 
	...
	public List<Integer> boardingGate(List<Integer> seatNumberList) {
		...
		for (Integer group1 : seatNumberList) {
			if (group1 <= 10 && group1 > 0) {
				myList.add(group1);
			}
		}
		for (Integer group2 : seatNumberList) {
			if (group2 <= 20 && group2 > 10) {
				myList.add(group2);
			}
		}
		for (Integer group3 : seatNumberList) {
			if (group3 <= 30 && group3 > 20) {
				myList.add(group3);
			}
		}
		return myList;
